...
myexp2 %>% 
  summarise(across(where(is.numeric), list(m = mean, s = sd)))
...
mydat <- tmpa %>% 
  left_join(tmpb, by=c("group_name", "name"))
...
```{r, eval=F, echo=F}
mydat %>% 
  ggplot(aes(x=name, y=mean, fill=group_name)) +
  geom_bar(stat="identity", position="dodge") +
  geom_errorbar(aes(ymin=mean-sd, ymax=mean+sd),
                position=position_dodge(width=0.9), 
                width=0.4, color="#555555") +
  scale_x_discrete(limit=mydat$name[1:10]) +
  theme_minimal() +
